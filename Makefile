...
release:
	GOARCH=amd64 GOOS=linux CGO_ENABLED=0 go build -buildmode=pie -ldflags "-X 'github.com/aws/rolesanywhere-credential-helper/cmd.Version=${VERSION}' -w -s" -trimpath -o build/bin/aws_signing_helper_amd64 main.go
	GOARCH=arm64 GOOS=linux CGO_ENABLED=0 go build -buildmode=pie -ldflags "-X 'github.com/aws/rolesanywhere-credential-helper/cmd.Version=${VERSION}' -w -s" -trimpath -o build/bin/aws_signing_helper_arm64 main.go
	file build/bin/aws_signing_helper_amd64
	file build/bin/aws_signing_helper_arm64
	ldd build/bin/aws_signing_helper_amd64
	ldd build/bin/aws_signing_helper_arm64
